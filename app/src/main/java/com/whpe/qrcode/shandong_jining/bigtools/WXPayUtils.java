    ...
    public void toWXPayNotSign(Context context) {
        ...
        Runnable payRunnable = new Runnable() {  //这里注意要放在子线程
            ...
            public void run() {
                ...
                System.out.println("----> check sign = " + request.checkArgs());

                boolean isSend = iwxapi.sendReq(request);//发送调起微信的请求
...
//                request.timeStamp = String.valueOf(genTimeStamp());
                ...
                System.out.println("----> check sign = " + request.checkArgs() + ", " + iwxapi.sendReq(request));
                boolean isSend = iwxapi.sendReq(request);//发送调起微信的请求
                System.out.println("----> is send  = " + isSend);
            }
        };
